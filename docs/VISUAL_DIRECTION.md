...
## Offline customization — milestone 16

The original single pilot above remains the approved preset and online/bot baseline. The Character screen adds bounded offline Base/Field/Scout artwork and independent head/hair/skin/clothing palettes, with added readable seams/cuffs/equipment accents. Fixed rig/silhouette, cyan/ochre identity, weapon grips/rays and all animation contracts remain. The large native UI preview uses the gameplay pose builder; at minimum size it shares the width with the action card. Error copy takes priority over the keyboard hint so actions remain reachable. Baseline, saved and draft atlases occupy three fixed slots, refreshed only on value changes. Exact catalog, provenance, save path and evidence are in [handoff 16](handoffs/16-character-customization.md); no full web-catalog or online parity is implied.
